...
def _whatsapp_poll_loop():
    ...
    while True:
        # Kill any stale Chromium processes holding the profile lock
...
def _check_plan_decision(path: Path, vault: VaultIO):
    ...
    if approved:
        try:
            # Guard: skip if file was already moved out of Plans/ by a previous event
            if not path.exists():
                log.info("[Plans] %s already moved — skipping duplicate event.", path.name)
                return
...
class PlansHandler(FileSystemEventHandler):
    ...
    def on_moved(self, event):
        # Obsidian saves files atomically (write temp → rename) which fires on_moved
        if not event.is_directory:
